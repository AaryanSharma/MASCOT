"""
Early-rank recall and trade-off analysis for VG_hour and I1M_geo.
Run from: results/
...
# ==========================================
STYLE_MAP = {
    "MA-SMF (Ours)":      {"color": "#003f5c", "marker": "*", "ls": "-",  "lw": 3.5, "s": 350, "z": 10},
    "MA-SMF (w/o Norm)":  {"color": "#4292c6", "marker": "X", "ls": "--", "lw": 2.5, "s": 200, "z": 9},
    "MA-SMF (w/o Omega)": {"color": "#9ecae1", "marker": "P", "ls": "--", "lw": 2.5, "s": 200, "z": 9},
...
METHOD_KEYS = {
    ...
    "14_ma_smf":                   "MA-SMF (Ours)",
    "15_ma_smf_ablation_no_norm":  "MA-SMF (w/o Norm)",
    "16_ma_smf_ablation_no_omega": "MA-SMF (w/o Omega)",
    "17_blip2":                    "BLIP-2",
    "18_mmr":                      "MMR",
...
DETAILED_METHOD_MAP = {
    ...
    "ma_smf_ablation_no_norm":  "MA-SMF (w/o Norm)",
    "ma_smf_ablation_no_omega": "MA-SMF (w/o Omega)",
    "ma_smf":                   "MA-SMF (Ours)",
    "blip2":                    "BLIP-2",
    "mmr":                      "MMR",
...
DATASETS = {
    "vg":  {"table_glob": "VG_*.json",  "label": "VG_hour"},
    "i1m": {"table_glob": "I1M_*.json", "label": "I1M_geo"},
}
...
# ==========================================
# PART 1: PARETO TRADE-OFF PLOTS
# ==========================================
sns.set_theme(style="whitegrid")

for ds_key, ds_info in DATASETS.items():
    table_dir = Path(ds_key) / "tables"
    if not table_dir.exists():
        print(f"Skipping trade-off for {ds_key}: {table_dir} not found")
        continue

    for table_path in table_dir.glob(ds_info["table_glob"]):
        ...
        for key, label in METHOD_KEYS.items():
            ...
            ax.scatter(entry["div_index"], entry["ret_index"], label=label,
                       ...
                       alpha=1.0 if "Ours" in label else 0.85, zorder=style["z"])
...
# ==========================================
# PART 2: EARLY-RANK RECALL PLOTS
# ==========================================
for ds_key, ds_info in DATASETS.items():
    fc_dir = Path(ds_key) / "failure_cases"
    if not fc_dir.exists():
        print(f"Skipping recall for {ds_key}: {fc_dir} not found")
        continue

    results = defaultdict(dict)
    sorted_keys = sorted(DETAILED_METHOD_MAP.keys(), key=len, reverse=True)

    for file in fc_dir.glob("failure_cases_*.json"):
        with open(file) as f:
            cases = json.load(f)

        parts = file.stem.split("_")
        if "increase" in parts:
            idx = parts.index("increase")
        elif "decrease" in parts:
            idx = parts.index("decrease")
        else:
            continue
        ...
        if method:
            results[dataset][method] = compute_recall_at_k(cases, RANKS)

    # Print tables
    print(f"\n=== EARLY-RANK RECALL: {ds_info['label'].upper()} ===")
    ...
    for dataset, methods in results.items():
        ...
        for method in sorted(methods.keys(), key=lambda x: STYLE_MAP[x]["z"]):
            recall = methods[method]
            vals = [recall[k] for k in RANKS]
            style = STYLE_MAP[method]
            plt.plot(RANKS, vals, label=method,
                     color=style["color"], marker=style["marker"],
