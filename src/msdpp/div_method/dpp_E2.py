...
class DPPGreedy(BaseDiversificationMethod):
    ...
    def calc_similarity_matrix(
        ...
        if self.use_ext:
            ext_sim = self.ext_kernel_matrix(info)
            ext_log = self._logm_pd(ext_sim)
            m_ext_log = ext_log.mean(0)
        else:
            m_ext_log = None

        if self.use_img:
            if img_feats is None:
                raise ValueError("img_feats must be provided when use_img=True")
            img_sim = self.img_kernel_matrix(img_feats)
            img_log = self._logm_pd(img_sim)
        else:
            img_log = None

        # ---- combine based on enabled attributes ----
        if self.use_ext and self.use_img:
            mean_log = sign * (1 - self.beta) * m_ext_log + self.beta * img_log
        elif self.use_ext:
            mean_log = sign * m_ext_log
        elif self.use_img:
            mean_log = sign * img_log
        else:
            raise ValueError("At least one of use_ext or use_img must be True")

        sim = self._expm(mean_log)
